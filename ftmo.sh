...
PYTHON="/root/.mt5/drive_c/users/root/AppData/Local/Programs/Python/Python38-32/python.exe"
WINE="wine"
...
start_bot() {
    ...
    if is_running "FTMO_V1.py"; then
        ...
        PYTHONIOENCODING=utf-8 PYTHONUTF8=1 /root/ftmo-env/bin/python $BOT_SCRIPT \
            >> "$BOT_LOG" 2>&1 &
        sleep 2
        if is_running "FTMO_V1.py"; then
...
start_dashboard() {
    ...
    if is_running "dashboard_api.py"; then
        ...
        PYTHONIOENCODING=utf-8 PYTHONUTF8=1 /root/ftmo-env/bin/python $DASH_SCRIPT \
            >> "$DASH_LOG" 2>&1 &
        sleep 3
        if is_running "dashboard_api.py"; then
            echo -e "${GREEN}  ✅ Dashboard started (PID: $(get_pid 'dashboard_api.py'))${NC}"
            echo -e "${CYAN}  🌐 Access at: http://localhost:5002${NC}"
        else
...
show_status() {
    ...
    if is_running "dashboard_api.py"; then
        ...
        echo -e "${CYAN}  🌐 http://localhost:5002${NC}"
    else
...
case "$1" in
    start)
        ...
        sleep 2
        brave-browser --new-window http://localhost:5002 \
            --no-first-run \
    ...
    browser)
        ...
        echo -e "${CYAN}  🌐 Dashboard: http://localhost:5002${NC}"
        ;;
    *)
