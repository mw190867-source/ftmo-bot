...
C = {
    ...
    "ORIGIN_SIGNAL": (Fore.CYAN,    Style.BRIGHT),
...
POST_CLOSE_COOLDOWN_SECS = 1500           # 25 min block on same symbol after any close
_symbol_close_cooldown: dict = {}         # symbol → epoch timestamp of last close
...
_consecutive_losses: int   = 0           # equity-curve risk scaling
_risk_scale_cooldown_until = None        # datetime (UTC-aware) or None
...
SYMBOL_CONFIG = {
    ...
    "EURUSD": {
        ...
        "cooldown": 300 if TESTING_MODE else 600,
        "max_trades": 3,                # raised from 2
        "max_trades_per_day": 3,        # added
        ...
        "score_min_pullback": 4,        # raised from 2
        "score_min_breakout": 5,        # raised from 3
        "score_min_continuation": 6,    # raised from 4
...
def reset_daily_trackers():
    global _daily_reset_date, daily_open_equity, daily_halt, daily_trade_counts, \
           _consecutive_losses, _risk_scale_cooldown_until, RISK_PERCENT
    today = date.today()
...
def find_xau_origin_level(rates_h1, atr):
    """Scan the last 50 H1 candles for the most recent impulsive origin candle
    (body > 2.0×ATR). Returns a dict or None."""
    if rates_h1 is None or len(rates_h1) < 5:
        return None
    lookback = min(50, len(rates_h1) - 2)
    for i in range(len(rates_h1) - 2, len(rates_h1) - 2 - lookback, -1):
        if i < 0:
            break
        c    = rates_h1[i]
        ...
        if body > 2.0 * atr:
            return {
                "origin_price":     float(c["open"]),
                "origin_high":      float(c["high"]),
                "origin_low":       float(c["low"]),
                "origin_direction": "BULL" if float(c["close"]) > float(c["open"]) else "BEAR",
                "origin_idx":       i,
            }
    return None
...
def get_signal(symbol):
        ...
        if rates_ok(rates_entry_r, 3):
            ...
            if candle_confirmation(direction, rates_entry_r, atr_entry, is_asia=asia, symbol=symbol):
                ...
                if not _sweep_prerequisite_met(symbol, direction):
                    # [SWEEP_FALLBACK] — allow XAUUSD without confirmed sweep when score>=7,
                    # HTF trend aligns, and H1 EMA50 confirms the same direction.
                    _sweep_fallback_ok = False
                    if symbol == "XAUUSD" and score >= 7:
                        _htf_aligned = (
                            (htf_trend == "BULL" and direction == "BUY") or
                            (htf_trend == "BEAR" and direction == "SELL")
                        )
                        if _htf_aligned:
                            _h1r = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_H1, 0, 60)
                            if rates_ok(_h1r, 55):
                                _h1c = np.array([r["close"] for r in _h1r], dtype=float)
                                _h1_ema50 = calculate_ema(_h1c, 50)
                                if _h1_ema50 is not None:
                                    _h1_bull = _h1c[-1] > _h1_ema50
                                    _h1_bear = _h1c[-1] < _h1_ema50
                                    if (direction == "BUY" and _h1_bull) or (direction == "SELL" and _h1_bear):
                                        _sweep_fallback_ok = True
                                        cp("WARNING",
                                           f"⚠️  [SWEEP_FALLBACK] {symbol} | no confirmed sweep but "
                                           f"score={score}>=7, HTF={htf_trend}, H1 EMA50 confirms {direction}")
                                        logger.info(
                                            "[SWEEP_FALLBACK] %s | dir=%s | score=%d | HTF=%s | H1_EMA50 confirmed",
                                            symbol, direction, score, htf_trend)
                    if not _sweep_fallback_ok:
    ...
    # --- MODE 4: ORIGIN OF MOVEMENT (XAUUSD CORRECTIVE only) ---
    if symbol == "XAUUSD" and _current_gold_regime == "CORRECTIVE":
        rates_h1_orig = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_H1, 0, 55)
        if rates_ok(rates_h1_orig, 52):
            origin = find_xau_origin_level(rates_h1_orig, atr)
            if origin is not None:
                origin_price = origin["origin_price"]
                origin_high  = origin["origin_high"]
                origin_low   = origin["origin_low"]
                # Direction determined by HTF, not BOS type
                if htf_trend == "BULL":
                    orig_dir = "BUY"
                elif htf_trend == "BEAR":
                    orig_dir = "SELL"
                ...
                if orig_dir is not None:
                    # Price must have returned to within 0.5 ATR of origin_price
                    if orig_dir == "BUY":
                        near_origin = origin_price <= price <= origin_price + 0.5 * atr
                    else:
                        near_origin = origin_price - 0.5 * atr <= price <= origin_price
                    if near_origin:
                        # Check M5 rejection candle: wick > 1.5x body (pin bar / hammer)
                        last_m5   = rates_signal[-2]
                        ...
                        if orig_dir == "BUY":
                            lower_wick = min(m5_open, m5_close) - m5_low
                            rejection  = m5_body > 0 and lower_wick > 1.5 * m5_body
                        else:
                            upper_wick = m5_high - max(m5_open, m5_close)
                            rejection  = m5_body > 0 and upper_wick > 1.5 * m5_body
                        if rejection:
                            origin_score = 6
                            # +1 if H1 EMA50 confirms direction
                            h1c = np.array([r["close"] for r in rates_h1_orig], dtype=float)
                            h1_ema50 = calculate_ema(h1c, 50)
                            if h1_ema50 is not None:
                                if (orig_dir == "BUY"  and h1c[-1] > h1_ema50) or \
                                   (orig_dir == "SELL" and h1c[-1] < h1_ema50):
                                    origin_score += 1
                            if origin_score >= 6:
                                # SL: 1.0 ATR beyond origin candle; TP: far edge of origin candle
                                if orig_dir == "BUY":
                                    custom_sl = origin_low  - atr
                                    custom_tp = origin_high
                                else:
                                    custom_sl = origin_high + atr
                                    custom_tp = origin_low
                                dist = abs(price - origin_price)
                                cp("ORIGIN_SIGNAL",
                                   f"[ORIGIN_SIGNAL] {symbol} | {orig_dir} | origin={origin_price:.2f}"
                                   f" | dist={dist:.2f} | score={origin_score} | HTF={htf_trend}"
                                   f" | regime={_current_gold_regime}")
                                logger.info(
                                    "[ORIGIN_SIGNAL] %s | dir=%s | origin=%.2f | dist=%.2f"
                                    " | score=%d | HTF=%s | regime=%s",
                                    symbol, orig_dir, origin_price, dist,
                                    origin_score, htf_trend, _current_gold_regime)
                                return orig_dir, price, {
                                    **meta_base,
                                    "score":      origin_score,
                                    "entry_mode": "origin",
                                    "custom_sl":  custom_sl,
...
def execute_trade(symbol, direction, meta):
    ...
    # POST_CLOSE_COOLDOWN — block new entries for 25 min after any close on this symbol
    _pcc_last = _symbol_close_cooldown.get(canonical_symbol(symbol), 0)
    _pcc_remaining = POST_CLOSE_COOLDOWN_SECS - (now - _pcc_last)
    if _pcc_remaining > 0:
        cp("WARNING",
           f"⚠️  {symbol} | [POST_CLOSE_COOLDOWN] {_pcc_remaining/60:.1f}min remaining after last close")
        logger.info("[POST_CLOSE_COOLDOWN] %s | %.1fmin remaining", symbol, _pcc_remaining / 60)
        _gate_hit(symbol, "post_close_cooldown"); return

    # RISK_COOLDOWN — block new entries during post-loss 1-hour cooldown window
    if _risk_scale_cooldown_until is not None:
        _rcd_now  = datetime.utcnow().replace(tzinfo=pytz.UTC)
        _rcd_mins = (_risk_scale_cooldown_until - _rcd_now).total_seconds() / 60
        if _rcd_mins > 0:
            cp("WARNING",
               f"⚠️  {symbol} | [RISK_COOLDOWN] {_rcd_mins:.1f}min remaining"
               f" after 3 consecutive losses (risk={RISK_PERCENT*100:.2f}%)")
            logger.info("[RISK_COOLDOWN] %s | %.1fmin remaining | risk=%.4f",
                        symbol, _rcd_mins, RISK_PERCENT)
            _gate_hit(symbol, "risk_cooldown"); return
    ...
    # ORIGIN mode — use pre-computed SL/TP stored in meta rather than the standard formula
    if entry_mode == "origin" and meta.get("custom_sl") and meta.get("custom_tp"):
        sl = normalize_price(symbol, float(meta["custom_sl"]))
        tp = normalize_price(symbol, float(meta["custom_tp"]))
    else:
...
def _update_equity_curve_risk(profit):
    """Adjust RISK_PERCENT based on consecutive losses. Called after every close."""
    global _consecutive_losses, _risk_scale_cooldown_until, RISK_PERCENT
    if profit < 0:
        _consecutive_losses += 1
        if _consecutive_losses >= 3:
            RISK_PERCENT = 0.0015
            _risk_scale_cooldown_until = (
                datetime.utcnow().replace(tzinfo=pytz.UTC) + timedelta(hours=1)
            )
            cp("WARNING",
               f"⚠️  [RISK_SCALED_DOWN] level=0.15% reason=3_consecutive_losses cooldown=1hr"
               f" | consec={_consecutive_losses}")
            logger.warning(
                "[RISK_SCALED_DOWN] level=0.15%% reason=3_consecutive_losses"
                " cooldown=1hr | consec=%d", _consecutive_losses)
        elif _consecutive_losses >= 2:
            RISK_PERCENT = 0.0025
            cp("WARNING",
               f"⚠️  [RISK_SCALED_DOWN] level=0.25% reason=2_consecutive_losses"
               f" | consec={_consecutive_losses}")
            logger.warning(
...
def close_position(pos, reason):
    ...
    if res and res.retcode == mt5.TRADE_RETCODE_DONE:
        ...
        _symbol_close_cooldown[canonical_symbol(pos.symbol)] = time.time()
    else:
...
def _finalise_close(ticket, known_symbol, deal, meta=None):
    ...
    # [POST_CLOSE_COOLDOWN] — record close epoch for per-symbol entry block
    if known_symbol and known_symbol != "?":
        _symbol_close_cooldown[canonical_symbol(known_symbol)] = time.time()
    if deal is not None:
...
def detect_closed_trades():
    ...
    for ticket in list(open_trades.keys()):
        if ticket not in existing and ticket not in _pending_deal_lookup:
            ...
            # [POST_CLOSE_COOLDOWN] — start cooldown immediately on detection (before deal history resolves)
            _cs = canonical_symbol(meta_snapshot.get("symbol", "?"))
            if _cs != "?":
                _symbol_close_cooldown[_cs] = time.time()
    ...
    if last_heartbeat_log_utc is None or (now_utc - last_heartbeat_log_utc).total_seconds() >= HEARTBEAT_INTERVAL_SEC:
        ...
        _phoenix_emit("HEARTBEAT", "ALL", heartbeat_msg,
                      ...
                      metadata={"uptime": uptime_str, "open_trades": len(open_trades),
                                "current_risk_pct": round(RISK_PERCENT * 100, 3),
    ...
    if last_telegram_heartbeat_utc is None or (now_utc - last_telegram_heartbeat_utc).total_seconds() >= TELEGRAM_HEARTBEAT_INTERVAL_SEC:
        try:
            ...
            if acc:
                ...
                _rsk_line = (
                    f"Risk: <b>{RISK_PERCENT*100:.2f}%</b>"
                    f" | Consec losses: <b>{_consecutive_losses}</b>"
                    + (" | ⏳ cooldown active" if _risk_scale_cooldown_until and
                       datetime.utcnow().replace(tzinfo=pytz.UTC) < _risk_scale_cooldown_until
                       else "")
                )
                tg_msg = (
                    f"💓 <b>Hourly Status</b>\n"
...
def check_offline_closures():
    ...
    try:
        deals = mt5.history_deals_get(search_from.replace(tzinfo=None), now_utc.replace(tzinfo=None))
        if not deals:
...
# WORKING FILE - FTMO V5.4 - VERIFIED Fri May 29 06:44:01 PM UTC 2026
